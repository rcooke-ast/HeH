...
writeit = True
dir = 'UVBfgal'
prefix = 'radiation_uvb_fgal'
#fracs = ["x", "-2.0", "-1.0", "-0.5", "+0.0", "+0.5", "+1.0", "+1.5", "+2.0", "+2.5", "+3.0"]
fracs = ["x", "-2.0", "-1.0", "-0.5", "+0.0", "+0.5", "+1.0"]#, "+1.5", "+2.0", "+2.5", "+3.0"]

prenams = ['fgal', '[C/H]', 'Hescl', 'nH', 'log NHI']
...
stchdata = None
for frac in fracs:
    fname = '{0:s}{1:s}'.format(prefix, frac.replace('-','m').replace('+','p').replace('.','d'))
    print('Preparing to stitch {0:s}'.format(dir + '/' + fname))
    metals, hescl, hden, NHI = np.loadtxt(dir + '/' + fname + '.grd', unpack=True, usecols=(6, 7, 8, 9))
    if frac == "x":
        slps = np.ones_like(metals) * -3.0  # Fudge
    else:
        slps = np.ones_like(metals) * float(frac)
